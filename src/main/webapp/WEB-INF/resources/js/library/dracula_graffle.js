 ...
 */
Raphael.fn.connection = function (obj1, obj2, style, type) {
    var selfRef = this;
    ...
    var edge = {/*
        ...
        draw : function() {
            ...
            var p = [
                ...
                {x: bb1.x - off1, y: bb1.y + bb1.height / 2-6},             /* WEST  1 */
                {x: bb1.x + bb1.width + off1, y: bb1.y + bb1.height / 2-6}, /* EAST  1 */
                {x: bb2.x + bb2.width / 2, y: bb2.y - off2},              /* NORTH 2 */
                {x: bb2.x + bb2.width / 2, y: bb2.y + bb2.height + off2}, /* SOUTH 2 */
                {x: bb2.x - off2, y: bb2.y + bb2.height / 2-3},             /* WEST  2 */
                {x: bb2.x + bb2.width + off2, y: bb2.y + bb2.height / 2-6}  /* EAST  2 */
            ];
            //we will fix position for them, not LOOP for all to find min.
       ...
       /*     for (var i = 0; i < 4; i++) { //modify this for x1,y1 is SOUTH, x4,y4 is NORTH
        ...
        /*    var res = dis.length == 0 ? [0, 4] : d[Math.min.apply(Math, dis).toFixed(3)];
			...
			var x1,y1,x4,y4;
			if(type == 1) //mean parent-children connection
			{x1 = p[0].x ;y1 = p[0].y ;x4 = p[5].x; y4 = p[5].y ;}
			else {x1 = p[2].x ;y1 =p[2].y ;x4 = p[7].x; y4 =p[7].y ;} // zer0 mean married people
			//x2,y2,x3,y3 for curve 
            //  path : TO - FROM
			var path = ["M", x1.toFixed(3), y1.toFixed(3), "L",(x4).toFixed(3), (y4).toFixed(3)].join(","); //this is for straight line, instead of curve as the comment (at under) xxxx
